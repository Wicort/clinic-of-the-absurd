...
public class TextConfigurationCreator
{
#if UNITY_EDITOR
    [MenuItem("Assets/Create/Game/Default Text Configuration")]
    public static void CreateDefaultTextConfiguration()
    {
        TextConfiguration config = ScriptableObject.CreateInstance<TextConfiguration>();
        
        // Заполняем гэги
        config.GagTexts = new GagTexts
        {
            Clownish = new string[]
            {
        ...
        // Заполняем реакции пациентов
        config.PatientReactions = new PatientReactionTexts
        {
            Angry = new string[]
            {
        ...
        config.UITexts = new UITexts
        {
            ...
            {
                "Товарищь игрок, на этом этаже еще остались пациенты, которых вы не посещали.\n" +
                "Вон, внизу двери открытые. Что, непонятно, что это двери?\n" +
                "Ну, простите, времени было мало, чтоб нарисовать понятнее..."
            }
        };
        
        // Заполняем диалоговые тексты
        config.DialogueTexts = new DialogueTexts
        {
        ...
        // Создаем ассет
        string path = "Assets/Resources/TextConfiguration.asset";
        AssetDatabase.CreateAsset(config, path);
        AssetDatabase.SaveAssets();
        AssetDatabase.Refresh();
        
        Debug.Log("TextConfiguration создан по пути: " + path);
    }
#endif
}
